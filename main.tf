...
######################
# Create a dedicated IAM Role for EBS CSI Service Account
resource "aws_iam_role" "ebs_csi_sa_role" {
  name = "devopsshack-ebs-csi-sa-role"
  ...
  "Statement": [
    {
      ...
      "Principal": {
        "Service": "eks.amazonaws.com",
        "Federated": "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/${aws_eks_cluster.devopsshack.identity[0].oidc[0].issuer}"
      },
      "Action": "sts:AssumeRoleWithWebIdentity",
      "Condition": {
        "StringEquals": {
          "${aws_eks_cluster.devopsshack.identity[0].oidc[0].issuer}:sub": "system:serviceaccount:kube-system:ebs-csi-controller-sa"
        }
      }
    }
  ]
...
resource "aws_iam_role_policy_attachment" "ebs_csi_sa_policy" {
